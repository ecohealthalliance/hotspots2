...
load(file.path(cache_dir(),"kfold_noweight_10x100_iter", paste0("kfold_noweight_10x100_iter", "_testing_events.RData")))recreated_folds <- kfold_gridids[[1]] %>% select(name, year, fold) %>% distinct()
...
predictions <- foreach(i = 1:length(kfm), .combine = "rbind") %do% {
  ...
  holdout_response <- predict(model_to_test, events_to_test, n.trees = model_to_test$n.trees, type = "response")

  predictions <- data.frame(reference = holdout_reference, response = holdout_response)
}
